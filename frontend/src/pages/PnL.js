...
const PnL = () => {
  ...
  const fetchUserStocks = async () => {
    try {
      const response = await axios.get(`https://leverage-x-12.onrender.com/api/users/stocks/${userId}`);
  ...
  const fetchRealTimePrices = async () => {
    try {
      ...
      const response = await axios.get(`https://leverage-x-12.onrender.com/api/watchlist${watchlistType}`);
  ...
  const handleSell = async (stockName, quantity, autoSell = false) => {
    try {
      ...
      const response = await axios.post('https://leverage-x-12.onrender.com/api/users/sell', {
        userId,
        stockName,
        quantity,
        watchlistType,
